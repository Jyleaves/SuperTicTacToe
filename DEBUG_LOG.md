...
| 位板 rollout | ~26万/s | SWAR popcount + de Bruijn ctz + xorshift，rollout 1.7x |
| **当前（位板 + greedy-1）** | **~27-31万/s** | 较最初纯 Python 版 **~127 倍** |

关键实验结论：
- fastmath 实测反而慢一倍——放弃；NODE_CAP 786k→262k 省 75MB（速率无差）
- C/ctypes 路线否决：ctypes 调用开销 ~10μs/次 > rollout 本身；全 C 重写记录为远期
...
- **256k 后无区分**：256k vs 512k 平 12/24、512k vs 1024k 平 19/24、512k vs 2048k（4 倍）平 10/12
  ——**棋力上限 ~256k-512k 迭代**（本机 0.8-1.6s/步）
...
| 困难 | 128,000 | 6.0s | ~500ms | 6s 截断 |
| 大师 | 256,000 | 12.0s | ~950ms | 12s 截断 |
...
- 实测：8 线程 164万/s（~10 倍加速近线性）；256k 高迭代结果与单线程一致
